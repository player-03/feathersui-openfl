...
@:keep
class FlowColumnsLayoutTest extends Test {
	private static final PADDING_TOP = 2.0;
	private static final PADDING_RIGHT = 10.0;
	private static final PADDING_BOTTOM = 8.0;
	private static final PADDING_LEFT = 6.0;
	private static final HORIZONTAL_GAP = 7.0;
	private static final VERTICAL_GAP = 5.0;
	private static final CHILD1_WIDTH = 100.0;
	private static final CHILD1_HEIGHT = 200.0;
	private static final CHILD2_WIDTH = 75.0;
	private static final CHILD2_HEIGHT = 150.0;
	private static final CHILD3_WIDTH = 50.0;
	private static final CHILD3_HEIGHT = 75.0;
	private static final CHILD4_WIDTH = 20.0;
	private static final CHILD4_HEIGHT = 10.0;

	private var _measurements:Measurements;
	private var _layout:FlowColumnsLayout;
	...
	public function setup():Void {
		this._layout = new FlowColumnsLayout();
	...
	public function testColumnHorizontalAlignChangeEvent():Void {
		var changed = false;
		this._layout.addEventListener(Event.CHANGE, function(event:Event):Void {
			changed = true;
		});
		Assert.isFalse(changed);
		this._layout.columnHorizontalAlign = RIGHT;
		Assert.isTrue(changed);
	}
	...
	public function testTwoItemsWithNullMeasurements():Void {
		var columnWidth = Math.max(CHILD1_WIDTH, CHILD2_WIDTH);
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
		Assert.equals(columnWidth, result.viewPortWidth);
		Assert.equals(CHILD1_HEIGHT + CHILD2_HEIGHT, result.viewPortHeight);
		Assert.equals(columnWidth, result.contentWidth);
	...
	public function testTwoItemsWithGapAndNullMeasurements():Void {
		...
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
		Assert.equals(columnWidth, result.viewPortWidth);
		Assert.equals(CHILD1_HEIGHT + CHILD2_HEIGHT + VERTICAL_GAP, result.viewPortHeight);
		Assert.equals(columnWidth, result.contentWidth);
		Assert.equals(CHILD1_HEIGHT + CHILD2_HEIGHT + VERTICAL_GAP, result.contentHeight);
		Assert.equals(0.0, result.contentX);
		Assert.equals(0.0, result.contentY);
	}
	...
	public function testTwoItemsWithPaddingAndNullMeasurements():Void {
		...
		var columnWidth = Math.max(CHILD1_WIDTH, CHILD2_WIDTH);
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + columnWidth, result.viewPortWidth);
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + CHILD1_HEIGHT + CHILD2_HEIGHT, result.viewPortHeight);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + columnWidth, result.contentWidth);
	...
	public function testTwoItemsWithPaddingGapAndNullMeasurements():Void {
		...
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + columnWidth, result.viewPortWidth);
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + CHILD1_HEIGHT + CHILD2_HEIGHT + VERTICAL_GAP, result.viewPortHeight);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + columnWidth, result.contentWidth);
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + CHILD1_HEIGHT + CHILD2_HEIGHT + VERTICAL_GAP, result.contentHeight);
		Assert.equals(0.0, result.contentX);
		Assert.equals(0.0, result.contentY);
	}

	public function testTwoItemsWithExplicitHeight():Void {
		this._measurements.height = Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT) * 1.25;
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
